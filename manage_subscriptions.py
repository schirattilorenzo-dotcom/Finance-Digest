...
--------------------------
Reads unread emails sent to infolsainews@gmail.com — checking both the
INBOX and the Spam folder, since command emails can occasionally get
filtered — and manages per-user RSS feed subscriptions based on the
email subject line (case-insensitive):
...
Processed emails are marked as read (\\Seen) in whichever folder they
were found in, so they aren't reprocessed on the next run. Unrecognized
subjects are left unread/untouched.
"""
import os
import csv
import json
...
# Folders to check for command emails. Confirm the exact Spam folder
# name for this account with the imap.list() snippet in the chat if
# this account's Gmail display language isn't English.
...
def process_inbox() -> None:
    ...
    # --- Phase 1: Retrieve unread commands from every folder, then disconnect ---
    ...
    for folder in FOLDERS_TO_CHECK:
        ...
        if status != "OK":
            print(f"Could not select folder '{folder}' — check its exact IMAP name. Skipping.")
            continue
 
        status, message_ids = imap.uid('search', None, "UNSEEN")
        if status != "OK":
            print(f"IMAP search failed in '{folder}'.")
            continue
    ...
    if not unread_emails:
        print("No unread command emails found in any checked folder.")
        return
 
    # --- Phase 2: Process messages in-memory ---
    success_uids = []  # list of (uid, folder) tuples
    for mail in unread_emails:
        ...
        print(f"Processing '{subject}' from {sender} (found in {folder})")
        ...
        try:
            if subject == "new":
                if sender in users:
                    ...
                    save_users(users)
                success_uids.append((uid, folder))
            ...
            print(f"Error processing email UID {uid_str} in '{folder}': {e}")
 
    # --- Phase 3: Reconnect to acknowledge successful emails, folder by folder ---
    if success_uids:
        try:
            imap = imaplib.IMAP4_SSL("imap.gmail.com")
            imap.login(INBOX_ADDRESS, INBOX_APP_PASSWORD)
            ...
            for uid, folder in success_uids:
            ...
            imap.logout()
            print(f"Successfully marked {len(success_uids)} emails as read across {len(by_folder)} folder(s).")
        except Exception as e:
